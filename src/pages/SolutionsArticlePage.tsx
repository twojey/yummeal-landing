...
import { articles, getArticle } from '../data/solutions';
...
export default function SolutionsArticlePage() {
  ...
  usePageMeta({
    ...
    jsonLd: article ? buildArticleJsonLd(article, `/solutions/${slug ?? ''}`) : undefined,
  });
    ...
    <div className="min-h-screen bg-[#FFFAF0] px-4 md:px-8 pt-24 pb-16">
      <div className="max-w-3xl mx-auto">
        <nav className="text-sm text-gray-500 mb-4">
          <Link to="/solutions" className="text-[#FF8C42] hover:underline">
            Solutions
          </Link>
        </nav>
        ...
        <SiloSiblings
          ...
          heading="Les autres solutions du quotidien"
        ...
        <RelatedArticles category="solutions" slug={article.slug} tags={article.tags ?? []}
        title={article.title} />
      </div>
    </div>
  );
