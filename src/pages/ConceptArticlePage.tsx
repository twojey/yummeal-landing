...
export default function ConceptArticlePage() {
  ...
  usePageMeta({
    ...
    canonicalPath: `/concept/${slug ?? ''}`,
    jsonLd: article ? buildArticleJsonLd(article, `/concept/${slug ?? ''}`) : undefined,
  });

  if (!article) {
    return <Navigate to="/concept" replace />;
  }
    ...
    <div className="min-h-screen bg-[#FFFAF0] px-4 md:px-8 pt-24 pb-16">
      <div className="max-w-3xl mx-auto">
        <nav className="text-sm text-gray-500 mb-4">
          <Link to="/concept" className="text-[#FF8C42] hover:underline">
            Concept
          </Link>
        </nav>
        ...
        <SiloSiblings
          ...
          heading="Les autres pages du concept"
        ...
        <RelatedArticles category="concept" slug={article.slug} tags={article.tags ?? []}
        title={article.title} />
      </div>
    </div>
  );
